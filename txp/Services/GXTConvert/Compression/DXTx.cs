...
namespace GXTConvert.Compression
{
    public static class DXTx
    {
        ...
        private static void DecodeDxtTile(BinaryReader reader, byte[] targetData, int x, int y, int width, int height,
            ...
            for (int by = 0; by < 8; by += 4)
            {
                for (int bx = 0; bx < 8; bx += 4)
                {
                    using (BinaryReader decodedReader =
                           new BinaryReader(new MemoryStream(DecompressDxtBlock(reader, format))))
                    {
        ...
        private static byte[] DecompressDxtColor(BinaryReader reader, SceGxmTextureBaseFormat format)
        {
            ...
            //Console.WriteLine("{0:x4} {1:x4}", color0, color1);
        ...
        private static byte[] DecompressDxtAlpha(BinaryReader reader, SceGxmTextureBaseFormat format)
        {
            ...
            else if (format == SceGxmTextureBaseFormat.UBC3)
            {
                ...
                ulong bits = (ulong)(((ulong)bits_0 << 40) | ((ulong)bits_1 << 32) | ((ulong)bits_2 << 24) |
                                     ((ulong)bits_3 << 16) | ((ulong)bits_4 << 8) | (ulong)bits_5);
