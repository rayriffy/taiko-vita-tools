...
{
    private static int MaxChunkSize = 0x10000;
    private static int PaddingBoundary = 0x200;

    public static void PackL7CA(string inputFoldername, string outputFilename)
    {
        ...
        using (BinaryWriter archiveWriter = new BinaryWriter(File.OpenWrite(outputFilename)))
        {
            ...
            foreach (var fs in filesystemEntries)
            {
                ...
                // Write padding
                archiveWriter.Write(new byte[(int)(PaddingBoundary -
                                                   ((archiveWriter.BaseStream.Length - PaddingBoundary) %
                                                    PaddingBoundary))]);
            ...
            header.archiveSize = (int)(header.metadataOffset + filesystemSection.Length + fileEntriesSection.Length +
                                       chunksSection.Length + stringSection.Length);
            header.metadataSize = (int)(filesystemSection.Length + fileEntriesSection.Length + chunksSection.Length +
    ...
    public static void UnpackL7CA(string inputFilename)
    {
        using (BinaryReader reader = new BinaryReader(File.OpenRead(inputFilename)))
        {
            ...
            for (int i = 0; i < header.filesystemEntries; i++)
            {
                ...
                if (entry.id == 0xffffffff)
                    ...
                    entry.filename = String.Format("{0}/{1}", strings[entry.folderOffset],
                        strings[entry.filenameOffset]);
            ...
            for (int i = 0; i < header.files; i++)
            {
                ...
                {
                    if (TaikoCompression.DebugDecompressionCode)
                    {
